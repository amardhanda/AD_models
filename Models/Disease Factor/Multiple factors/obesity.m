...
final_change = zeros(length(disease_factors) + 1,4);
...
for k = 1: 1
            ...
            count_dis = sum((vec_scr(1,34,:)./vec_scr(1,33,:).^2 >= 0.0427 & vec_scr(1,dis_fact_2,:) >= 6 & vec_scr(1,dis_fact_2,:) <= 80) );


            count_norm = sum(vec_scr(1,34,:)./vec_scr(1,33,:).^2 < 0.0427 & vec_scr(1,dis_fact_2,:) >= 0 & vec_scr(1,dis_fact_2,:) <= 5);
            ...
            %%
            for i = 1:l1
                if (vec_scr(1,34,i)./vec_scr(1,33,i).^2 >= 0.0427 & vec_scr(1,dis_fact_1,i) >= 6 & vec_scr(1,dis_fact_1,i) <= 80)
                    disease(:,:,count_dis) = vec_scr(:,:,i); 
                    count_dis = count_dis + 1;

                elseif (vec_scr(1,34,i)./vec_scr(1,33,i).^2 < 0.0427 & vec_scr(1,dis_fact_1,i) >= 0  & vec_scr(1,dis_fact_1,i) <= 5)
    ...
    for k = 1: length(disease_factors)
            ...
            count_dis = sum((vec_scr(1,34,:)./vec_scr(1,33,:).^2 >= 0.0427 & (vec_scr(1,dis_fact_2,:) == 1 | vec_scr(1,dis_fact_2,:) == 2)));


            count_norm = sum(vec_scr(1,34,:)./vec_scr(1,33,:).^2 < 0.0427 & (vec_scr(1,dis_fact_2,:) == 0));
            ...
            %%
            for i = 1:l1
                if (vec_scr(1,34,i)./vec_scr(1,33,i).^2 >= 0.0427 & (vec_scr(1,dis_fact_2,i) == 1 | vec_scr(1,dis_fact_2,i) == 2))
                    disease(:,:,count_dis) = vec_scr(:,:,i);
                    count_dis = count_dis + 1;

                elseif (vec_scr(1,34,i)./vec_scr(1,33,i).^2 < 0.0427 & (vec_scr(1,dis_fact_2,i) == 0))
                    normal(:,:,count_norm) = vec_scr(:,:,i);
                    count_norm = count_norm + 1;
