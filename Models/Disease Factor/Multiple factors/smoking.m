...
final_change = zeros(length(disease_factors),4);
...
j = 1;
    for k = 1: length(disease_factors)
            ...
            count_dis = sum((vec_scr(1,dis_fact_1,:) >= 6 & vec_scr(1,dis_fact_1,:) <= 80) & (vec_scr(1,dis_fact_2,:) == 1 | vec_scr(1,dis_fact_2,:) == 2));


            count_norm = sum(vec_scr(1,dis_fact_1,:) >= 0  & vec_scr(1,dis_fact_1,:) <= 5 & vec_scr(1,dis_fact_2,:) == 0);
            ...
            %%
            for i = 1:l1
                if (vec_scr(1,dis_fact_1,i) >= 6 & vec_scr(1,dis_fact_1,i) <= 80) & (vec_scr(1,dis_fact_2,i) == 1 | vec_scr(1,dis_fact_2,i) == 2)
                    disease(:,:,count_dis) = vec_scr(:,:,i);
                    count_dis = count_dis + 1;

                elseif (vec_scr(1,dis_fact_1,i) >= 0  & vec_scr(1,dis_fact_1,i) <= 5 & vec_scr(1,dis_fact_2,i) == 0)
